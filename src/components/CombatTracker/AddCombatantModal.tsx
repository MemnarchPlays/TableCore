...
export default function AddCombatantModal({ onClose }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setError('')
    document.getElementById('combatant-name-input')?.focus()
  }
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end justify-center sm:items-center">
      ...
      {/* Panel — max-height so it never overflows the viewport; flex-col so buttons pin to bottom */}
      <div className="relative z-50 w-full max-w-md max-h-[90dvh] flex flex-col bg-zinc-800 border border-zinc-700 rounded-t-2xl sm:rounded-2xl shadow-2xl">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 shrink-0">
          <h2 className="text-lg font-bold text-zinc-100">Add Combatant</h2>
          <button
            onClick={onClose}
        ...
        {/* Scrollable fields */}
        <form id="add-combatant-form" onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          <div className="overflow-y-auto flex-1 px-5 space-y-3 pb-2">
            <div>
              <label className="block text-xs font-medium text-zinc-400 mb-1">Name *</label>
              <input
                id="combatant-name-input"
                autoFocus
                type="text"
                value={name}
            ...
            {!isEnvironmental && (
              <div>
                <button
                  ...
                  className="text-xs text-zinc-500 hover:text-zinc-300 transition-colors flex items-center gap-1 min-h-[44px]"
                >
          ...
          {/* Action buttons — pinned at bottom, never scroll away */}
